...
const CourseDetailPage: React.FC = () => {
  ...
  if (!course) {
    return (
      <div
        className={`flex flex-col items-center justify-center h-full ${isDark ? "bg-gray-900 text-gray-300" : "bg-white text-gray-600"}`}
      >
        <p>Không tìm thấy môn học</p>
        <button
          onClick={() => navigate("/chat")}
  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto p-6">
        ...
        {/* Course Header */}
        <div
          className={`rounded-xl p-6 mb-6 ${isDark ? "bg-gray-800" : "bg-white shadow-sm"}`}
        >
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button
              onClick={() =>
                user?.role === "teacher"
                  ? navigate(`/teacher/quizzes/${courseId}`)
        ...
        {/* Help Text */}
        <div
          className={`mt-6 p-4 rounded-lg text-sm ${isDark ? "bg-gray-800/50 text-gray-400" : "bg-white/50 text-gray-500"}`}
        >
          <p>
            <strong>Mẹo:</strong> Bạn có thể hỏi AI về nội dung trong các tài
            liệu được đánh dấu "Đã xử lý". AI sẽ trích dẫn nguồn từ tài liệu
            gốc.
          </p>
        </div>
      </div>
